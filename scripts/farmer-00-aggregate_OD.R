...
library(tidyverse)
library(data.table)
setwd("/Users/cychang/Desktop/Lab/farmer_ecoli/scripts/")
...
dd <- rep(list(NA), length(file_names))
...
for(i in 1:length(dd)){
  ...
  # .. map wells from original community 96DW to 384 screening plates
  ...
  # Subtract median value of blanks from all wells
  ...
  d <- subtract.blanks(d)
  dd[[i]] <- cbind(transfer = sub("Farmer_", "", file_names[i]) %>% sub(".txt", "", .), d)
}

# .. combine the list to one single data frame
dd <- rbindlist(dd)
...
dd %>%
  ...
  fwrite('../data/temp/farmer_OD.txt')
